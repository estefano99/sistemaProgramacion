...
const tbody_tragos = document.querySelector(".tbody_tragos");
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    fetch("./APItragos.php") //Trae los tipos de tragos de la API
        .then(res => res.json())
...
function filtrarTipoDeTrago() {
    const resultado = nuevoDatos.filter(filtrarNombre).filter(filtrarPrecio).filter(filtrarTipo)
...
function generarHTML(datos) {
    ...
    datos.forEach(data => {
        const { id_tragos, nombre, tipo,precio, descripcion, favoritos, imagen } = data
        ...
        <td>${descripcion}</td>
        <td>${favoritos}</td>
        <td>${imagen ? `<img src="../imagenesTragos/${imagen}" width="100px" alt="imagen">` : 'No hay imagen'}</td>
        <td>
            <a href="modificarTragos.php?upd=${id_tragos}" ><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 iconos">
                <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                </svg>
            </a>
            <a href="eliminarTragos.php?usr=${id_tragos}"class="w-100 h-100"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 iconos">
...
btnInactivo.addEventListener("click", () => {
    fetch("./APIinactivos.php") //Trae los tipos de tragos de la API inactivos
        .then(res => res.json())
    ...
    datos.forEach(data => {
        ...
        <td>
            <form name='form' id='form' method='POST' action='tragosActivos.php'>
                <input type='hidden' value='${id_tragos}' name='id' id='id'>
...
function limpiarHTML() {
    while (tbody_tragos.firstChild) {
        tbody_tragos.removeChild(tbody_tragos.firstChild)
    }
}
